...
const formatProjectRoomAgentLabel = (displayLabel: string) => {
  const label = displayLabel.trim();
  const legacyFallbackMatch = /^Codex ([0-9a-f]{8})$/i.exec(label);

  if (legacyFallbackMatch) {
    return `Codex Agent · ${legacyFallbackMatch[1]}`;
  }
  if (/^Codex(?: Agent)? · /i.test(label)) {
    return label;
  }
  return label ? `Codex · ${label}` : "Codex Agent";
};
...
export const selectProjectRoomAgentPresence = (
  ...
  for (const participant of participants) {
    if (participant.role === "desktop-editor") {
      continue;
    }
    const current = byActorId.get(participant.actorId);
    if (
      !current ||
      (current.role === "agent-writer" && participant.role === "board-editor")
    ) {
...
export const createProjectRoomCollaborators = (
  ...
  for (const participant of selectProjectRoomAgentPresence(participants)) {
    ...
    collaborators.set(socketId, {
      ...
      socketId,
      username: formatProjectRoomAgentLabel(participant.displayLabel),
