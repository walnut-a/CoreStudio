...
- 中央 800 × 600 的 4:3 生成舞台是主视觉，初始、生成中与最终结果只在同一框内切换；左侧参考带作为输入，Slogan 位于左下作为次级说明。
...
桌面场景是一张固定的 1680 × 960 编辑平面。800 × 600 的 4:3 生成舞台位于画布中央偏右，是任何状态下都不移动的主对象；四张参考图压成左侧 480 × 132 的不等宽输入带，并通过一枚 Excalidraw 原生箭头指向舞台。Slogan 位于左下，以更小的近黑标题场承担次级品牌说明。四格细节带位于生成舞台下方，但首次进入时隐藏。固定于视口的品牌、下载、倍率和 composer 构成真实应用外壳。

镜头以当前视口的安全区计算构图倍率。90% 是常规桌面镜头的上限参考，不再作为所有桌面窗口的固定下限；普通桌面与 471–820px 紧凑窗口优先完整展示标题、参考带和主结果，超宽或高分辨率屏幕再提高倍率并向上收紧空白。初始、生成中与完成态始终沿用同一响应式镜头，不因状态变化自动放大结果；470px 以下保留可读的 40% 聚焦镜头，让标题完整出现并露出结果关系。画布继续支持单指平移、双指以手势中心缩放和点击选中。最小倍率以完整场景适配视口为准，并受生产级缩放下限约束。
...
- **Reference Sheet:** 四张授权参考图以一条不等宽素材带排列，不加卡片底与单张阴影；首次进入时用每张图片的独立细选框和一组共用控制点表达真实多选。
