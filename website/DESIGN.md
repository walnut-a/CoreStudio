...
桌面场景是一张固定的 1680 × 960 编辑平面。左侧 560 × 336 的四图参考组与右侧 560 × 373 的生成舞台垂直对齐，形成清楚的输入—输出双列关系；参考图以两行不等宽拼贴呈现，不再压成单行缩略带。两列之间保留 184px 场景通道，以一枚 144px 的延长 Excalidraw 双线箭头连接，让关系在响应式缩放后仍然舒展。生成舞台沿用素材原始 3:2 比例，以 `contain` 完整显示，不做额外放大裁切。Slogan 位于左下近黑标题场，右下与它共享一条内容带，依次放置本地项目、模型自由、Agent 写回三个利益点；这些产品事实从首次进入就可见，不再依赖生成完成后披露。固定于视口的品牌、下载、倍率和 composer 构成真实应用外壳。
...
- **Generated Result:** 结果图只出现一次，使用素材原始 3:2 比例完整缩放；选中与生成占位仍复用同一结果框，不增加裁切细节副本。
