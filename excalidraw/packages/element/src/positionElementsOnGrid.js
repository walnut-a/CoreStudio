...
exports.positionElementsOnGrid = void 0;
const bounds_1 = require("./bounds");
const mutateElement_1 = require("./mutateElement");
// TODO rewrite (mostly vibe-coded)
const positionElementsOnGrid = (elements, centerX, centerY, padding = 50) => {
    ...
    const atomicUnits = Array.isArray(elements[0])
        ? elements
    ...
    const rowProperties = rows.map((rowUnits) => {
        ...
        const unitBounds = rowUnits.map((unit) => {
            const [minX, minY, maxX, maxY] = (0, bounds_1.getCommonBounds)(unit);
            return {
                elements: unit,
    ...
    rowProperties.forEach((rowProp) => {
        ...
        unitBounds.forEach((unitBound) => {
            ...
            unitBound.elements.forEach((element) => {
                res.push((0, mutateElement_1.newElementWith)(element, {
                    x: element.x + offsetX,
                    y: element.y + offsetY,
                }));
            });
...
exports.positionElementsOnGrid = positionElementsOnGrid;
