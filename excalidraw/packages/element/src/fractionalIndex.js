...
exports.syncInvalidIndicesImmutable = exports.syncInvalidIndices = exports.syncMovedIndices = exports.orderByFractionalIndex = exports.validateFractionalIndices = exports.InvalidFractionalIndexError = void 0;
const fractional_indexing_1 = require("fractional-indexing");
...
class InvalidFractionalIndexError extends Error {
    code = "ELEMENT_HAS_INVALID_INDEX";
}
exports.InvalidFractionalIndexError = InvalidFractionalIndexError;
/**
 ...
 */
const validateFractionalIndices = (elements, { shouldThrow = false, includeBoundTextValidation = false, ignoreLogs, reconciliationContext, }) => {
    const errorMessages = [];
    ...
    for (const [i, index] of indices.entries()) {
        ...
        if (includeBoundTextValidation && (0, typeChecks_1.hasBoundTextElement)(elements[i])) {
            const container = elements[i];
            const text = (0, textElement_1.getBoundTextElement)(container, (0, common_1.arrayToMap)(elements));
...
exports.validateFractionalIndices = validateFractionalIndices;
/**
 * Order the elements based on the fractional indices.
 * - when fractional indices are identical, break the tie based on the element id
 * - when there is no fractional index in one of the elements, respect the order of the array
 */
const orderByFractionalIndex = (elements) => {
...
exports.orderByFractionalIndex = orderByFractionalIndex;
/**
 ...
 */
const syncMovedIndices = (elements, movedElements) => {
    try {
        ...
        (0, exports.validateFractionalIndices)(elementsCandidates, 
        ...
        for (const [element, { index }] of elementsUpdates) {
            (0, mutateElement_1.mutateElement)(element, elementsMap, { index });
        }
    }
    catch (e) {
        // fallback to default sync
        (0, exports.syncInvalidIndices)(elements);
    }
    return elements;
};
exports.syncMovedIndices = syncMovedIndices;
/**
 ...
 */
const syncInvalidIndices = (elements) => {
    const elementsMap = (0, common_1.arrayToMap)(elements);
...
exports.syncInvalidIndices = syncInvalidIndices;
/**
 ...
 * WARN: in edge cases it could modify the elements which were not moved, as it's impossible to guess the actually moved elements from the elements array itself.
 */
const syncInvalidIndicesImmutable = (elements) => {
    const syncedElements = (0, common_1.arrayToMap)(elements);
    const indicesGroups = getInvalidIndicesGroups(elements);
    const elementsUpdates = generateIndices(elements, indicesGroups);
    for (const [element, { index }] of elementsUpdates) {
        syncedElements.set(element.id, (0, mutateElement_1.newElementWith)(element, { index }));
    }
    return syncedElements;
};
exports.syncInvalidIndicesImmutable = syncInvalidIndicesImmutable;
/**
...
const generateIndices = (elements, indicesGroups) => {
    ...
    for (const indices of indicesGroups) {
        ...
        const fractionalIndices = (0, fractional_indexing_1.generateNKeysBetween)(elements[lowerBoundIndex]?.index, elements[upperBoundIndex]?.index, indices.length);
