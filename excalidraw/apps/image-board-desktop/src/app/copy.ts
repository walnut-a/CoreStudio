  ...
  applicationSettings: {
    ...
    aboutPage: {
      ...
      update: {
        ...
        failureDescriptions: {
          network: "无法连接更新服务，请检查网络连接或代理设置后重试。",
          timeout: "更新服务响应超时，请稍后重试。",
          serviceNotConfigured: "更新服务尚未正确部署，请稍后重试。",
          serviceUnavailable: "更新服务暂时不可用，请稍后重试。",
          serviceError: "更新服务返回异常，请稍后重试。",
          invalidResponse: "更新信息暂时无法识别，请稍后重试。",
          unsupported: "当前版本暂不支持检查更新。",
          unknown: "检查更新时发生未知错误，请稍后重试。",
        },
        openDownload: "前往下载",
        viewNotes: "查看更新说明",
      },
    ...
    codexPage: {
      hostStartPrompts: {
        workbuddy:
          "在 WorkBuddy 本地任务中使用 CoreStudio Skill，先读取 Skill 目录中的 references/host.md，检查当前工具并连接我选择的项目，验证画布与选区。能够完成的接入步骤由你执行；只有遇到登录、权限或必须手动连接的阻碍时，再告诉我具体需要做什么。暂不生成图片。",
        qwenwork:
          "在 千问办公 本地任务中使用 CoreStudio Skill，先读取 Skill 目录中的 references/host.md，检查当前工具并连接我选择的项目，验证画布与选区。能够完成的接入步骤由你执行；只有遇到登录、权限或必须手动连接的阻碍时，再告诉我具体需要做什么。暂不生成图片。",
        doubaowork:
          "在 豆包工作 本地任务中使用 CoreStudio Skill，先读取 Skill 目录中的 references/host.md，检查当前工具并连接我选择的项目，验证画布与选区。能够完成的接入步骤由你执行；只有遇到登录、权限或必须手动连接的阻碍时，再告诉我具体需要做什么。暂不生成图片。",
      },
      description:
      ...
      installPrompt: (appVersion: string, guideUrl: string) =>
        `请为 Codex 安装本机 CoreStudio ${appVersion} 集成。先读取接入指南 ${guideUrl}，优先使用应用包内 setup.sh 的无界面安装入口，不依赖 Computer Use，也不要从网络下载或重写安装脚本。若旧包没有该入口，再请用户在“应用设置 → Agent 集成”安装。完成后只验证 CLI、Skill 和兼容性记录，暂不访问项目或生成图片。`,
    },
  },
  agentUi: {
    ...
    imageAssetSearch: "搜索图片资产",
    imageAssetFilter: "按类型筛选图片资产",
    imageAssetFilterAll: "全部",
    imageAssetFilterGenerated: "生成",
    imageAssetFilterImported: "导入",
    imageAsset: {
      imported: "导入图片",
      untitledGenerated: "未命名生成",
      agentGenerated: "Agent 生成图片",
      coreStudioGenerated: "CoreStudio 生成图片",
      onBoard: "画布中",
      reference: "参考图",
      unused: "未使用",
    },
