  ...
  it("installs and executes the legacy and multi-host integrations from the packaged app", () => {
    ...
    const spawnSync = vi
      ...
      .mockReturnValueOnce({
        status: 0,
        stdout:
          '{"ok":true,"data":{"appVersion":"1.1.26","integrationVersion":"2.1.3","bridgeProtocolVersion":7}}\n',
        stderr: "",
      })
    ...
    runCodexIntegrationSmoke({
      ...
      readFileSync: (filePath) => {
        ...
        if (filePath.endsWith("agent-integration/contract.json")) {
          return '{"schemaVersion":2,"integrationVersion":"2.1.3","bridgeProtocolVersion":7,"skillVersion":22,"cliWrapperVersion":2,"hosts":["codex","cursor","claude-code"]}';
        }
