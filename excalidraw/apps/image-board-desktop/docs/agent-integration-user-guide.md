...
在支持开放项目的运行时，项目原图可以从文件夹外部添加，顺序与坐标保存在公开的 `project.json.layout`，完整原生场景仍保存在 `scene.excalidraw.json`。只改顺序不会重新排布，后台接纳也不会移动已整理的图片。不要另建旧图片索引、接纳文件或 layout 历史文件。

让 Agent 整理已有图片时，说明或沿用已确认的排序依据。已存 `layout.order` 是整理结果，素材记录时间可能由同批导入共用，都不等于真实添加先后。选择文件创建时间时按原图 birth time 排序，同时间按文件名和相对路径确定顺序；无法还原添加时间时应说明，不静默改用其他依据。需要改变画布排布时一起更新公开顺序和坐标，由 Bridge 同步，并核对原生场景及保存状态。Skill 26 补充了这条流程；使用前通过应用集成安装器更新本机 Skill。

Agent 通过 `read capabilities` 检查 `openProject`，再使用同一任务 session 的 `read status` 查看 `projectRoom.storage`。Bridge 正常不代表文件同步正常；`blocked` 时保留错误与原请求 ID，在对应项目中点击“处理文件变化”查看差异和选择版本。旧客户端缺少字段时状态未知；CLI 不提供强制覆盖命令。需要图片关联证据时读取 `read health`，身份或清单不可读时先从桌面恢复。

项目窗口仍保有画布时，可以据此恢复丢失文件；仅剩原图时，从文件夹重建会创建新项目身份，无法还原旧坐标、文字、删除意图和生成参数，Agent 需要重新认领。安装旧版客户端不会因为源码或在线文档更新而获得这些能力；更新应用后再通过应用内集成安装器更新受管 Skill。
...
先确认应用包内集成合同包含对应宿主，或由用户在设置中确认；无需为了检查而操作窗口。没有该宿主的旧包需要更新。官网和仓库源码更新不代表本机应用已更新。集成合同为 2.2.2，公共 Skill 为 26；具体 Skill 修订以当前应用检测为准。
