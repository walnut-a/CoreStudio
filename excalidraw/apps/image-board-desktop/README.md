...
Use `scene image-paths` when an Agent needs local reference files. It returns the original image paths from the current project instead of streaming image bytes through the CLI. Prefer `--selection` or `--file-ids`; `--all` is explicit because large projects can contain many images.

Generation mode is an explicit user preference. `agent context --json` returns `generation.source`:
...
ACP text output is display-only. Project mutations must still go through the CoreStudio CLI / Local Bridge, for example `scene add-image`, `scene add-prompt`, or `generate`. This keeps CoreStudio's local project format as the single source of truth.
...
When the Agent Board is open, it publishes its current selection and viewport to the bridge. CLI write commands then use that runtime context for reference selection and placement, so `generate --use-selection` targets the embedded board state rather than the desktop window's local selection.

Use `--dry-run` with `scene add-prompt` or `scene add-image` to validate a write payload without mutating the board. Dry-run responses summarize image payloads without echoing `dataBase64`.
