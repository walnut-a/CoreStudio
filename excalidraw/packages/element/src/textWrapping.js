...
exports.getWrappedTextLines = exports.wrapText = exports.parseTokens = exports.containsCJK = void 0;
...
const wrapText = (text, font, maxWidth) => {
    return (0, exports.getWrappedTextLines)(text, font, maxWidth)
        .map((line) => line.text)
        .join("\n");
};
 ...
 */
const getWrappedTextLines = (text, font, maxWidth) => {
    ...
    for (const originalLine of text.split("\n")) {
        const originalLineWidth = (0, textMeasurements_1.getLineWidth)(originalLine, font);
...
exports.getWrappedTextLines = getWrappedTextLines;
/**
 ...
 */
const wrapLine = (line, font, maxWidth, lineStart) => {
    const lines = [];
    const tokens = (0, exports.parseTokens)(line);
    let currentLine = "";
    ...
    while (tokenIndex < tokens.length) {
        ...
        const testLineWidth = isSingleCharacter(token)
            ? currentLineWidth + textMeasurements_1.charWidth.calculate(token, font)
            : (0, textMeasurements_1.getLineWidth)(testLine, font);
        ...
        if (!currentLine) {
            ...
            currentLineWidth = (0, textMeasurements_1.getLineWidth)(trailingLine.text, font);
            tokenOffset = tokenEnd;
            tokenIndex++;
        }
        else {
...
const wrapWord = (word, font, maxWidth, wordStart) => {
    ...
    for (const char of chars) {
        ...
        const _charWidth = textMeasurements_1.charWidth.calculate(char, font);
 ...
 */
const trimLine = (line, start, end, font, maxWidth) => {
    const shouldTrimWhitespaces = (0, textMeasurements_1.getLineWidth)(line, font) > maxWidth;
    ...
    let trimmedLineWidth = (0, textMeasurements_1.getLineWidth)(trimmedLine, font);
    for (const whitespace of Array.from(whitespaces)) {
        const _charWidth = textMeasurements_1.charWidth.calculate(whitespace, font);
 ...
 * text.
 */
const trimLineEndAtSoftBreak = (line, start, end) => {
