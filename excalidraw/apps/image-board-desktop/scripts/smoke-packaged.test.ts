  ...
  it("installs and executes the Codex integration from the packaged app", () => {
    ...
    const spawnSync = vi
      ...
      .mockReturnValueOnce({
        status: 0,
        stdout:
          '{"ok":true,"data":{"appVersion":"1.1.20","integrationVersion":"1.1.0","bridgeProtocolVersion":1}}\n',
        stderr: "",
      });
    const rmSync = vi.fn();
      ...
      readFileSync: (filePath) =>
        filePath.endsWith("CODEX_INSTALLATION.md")
          ...
          : filePath.endsWith("corestudio-integration.json")
            ? '{"installedFromAppVersion":"1.1.20","integrationVersion":"1.1.0","bridgeProtocolVersion":1}'
            : '{"version":"1.1.20"}',
      rmSync,
      spawnSync,
      tmpdir: () => "/tmp",
