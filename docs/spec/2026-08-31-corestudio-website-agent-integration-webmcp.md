...
- 首页增加清晰的可见入口，并注册与集成中心相同的只读教程工具；不在视觉层承载完整教程，不破坏当前单视口画布叙事。
...
- 支持 WebMCP 的浏览器 Agent 能够从官网首页或集成中心读取与可见教程一致的结构化内容。
...
首页同时注册集成中心的三项只读 WebMCP 教程工具。WebMCP 工具属于当前文档，浏览器 Agent 只打开首页时不会继承子页面注册的能力；因此可见入口和机器可发现入口必须同时存在。工具返回的 `pageUrl` 继续指向对应语言和章节的集成中心，首页不复制教程正文。
...
集成中心必须先是一份完整教程，官网工具只是同一内容的结构化表达。英文和中文首页也注册同一组工具，确保 Agent 从官网入口页即可发现能力；页面跳转后由新文档重新注册，不依赖子页面工具跨页面继承。Agent Board 工具则使用当前已同步的 Project Room 状态和现有 renderer 动作，不新建第二份项目数据源。
...
- 首页仅增加可见入口、WebMCP 注册标记和共享适配器脚本，不复制教程正文，也不让文档布局规则进入 `canvas-engine.mjs`。
...
- [ ] 支持 WebMCP 的目标浏览器在英文首页、中文首页和对应集成页都能发现相同的三个只读工具。
...
- 在首页增加 Agent 集成入口，并挂载共享 WebMCP 适配器。
