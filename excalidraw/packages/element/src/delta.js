...
exports.ElementsDelta = exports.AppStateDelta = exports.Delta = void 0;
...
class Delta {
    ...
    static create(deleted, inserted, modifier, modifierOptions) {
        const modifiedDeleted = modifier && modifierOptions !== "inserted"
            ? modifier(deleted, "deleted")
            : deleted;
        const modifiedInserted = modifier && modifierOptions !== "deleted"
     ...
     */
    static merge(delta1, delta2, delta3 = Delta.empty()) {
        return Delta.create({ ...delta1.deleted, ...delta2.deleted, ...delta3.deleted }, { ...delta1.inserted, ...delta2.inserted, ...delta3.inserted });
    }
    /**
    ...
    static mergeArrays(prev, added, removed, predicate) {
        return Object.values(Delta.mergeObjects((0, common_1.arrayToObject)(prev ?? [], predicate), (0, common_1.arrayToObject)(added ?? [], predicate), (0, common_1.arrayToObject)(removed ?? [], predicate)));
    }
    /**
    ...
    static diffArrays(deleted, inserted, property, groupBy) {
        ...
        if (Array.isArray(deleted[property]) || Array.isArray(inserted[property])) {
            ...
            const deletedDifferences = (0, common_1.arrayToObject)(Delta.getLeftDifferences((0, common_1.arrayToObject)(deletedArray, groupBy), (0, common_1.arrayToObject)(insertedArray, groupBy)), (x) => x);
            const insertedDifferences = (0, common_1.arrayToObject)(Delta.getRightDifferences((0, common_1.arrayToObject)(deletedArray, groupBy), (0, common_1.arrayToObject)(insertedArray, groupBy)), (x) => x);
            if (Object.keys(deletedDifferences).length ||
                Object.keys(insertedDifferences).length) {
    ...
    static isInnerDifferent(object1, object2, skipShallowCompare = false) {
        const anyDistinctKey = !!this.distinctKeysIterator("inner", object1, object2, skipShallowCompare).next().value;
        return !!anyDistinctKey;
    }
    /**
    ...
    static isDifferent(object1, object2, skipShallowCompare = false) {
        const anyDistinctKey = !!this.distinctKeysIterator("full", object1, object2, skipShallowCompare).next().value;
        return !!anyDistinctKey;
    }
    /**
    ...
    static getLeftDifferences(object1, object2, skipShallowCompare = false) {
        return Array.from(this.distinctKeysIterator("left", object1, object2, skipShallowCompare)).sort();
    }
    /**
     * Returns sorted object2 keys that have distinct values.
     */
    static getRightDifferences(object1, object2, skipShallowCompare = false) {
        return Array.from(this.distinctKeysIterator("right", object1, object2, skipShallowCompare)).sort();
    }
    /**
    ...
    static getInnerDifferences(object1, object2, skipShallowCompare = false) {
        return Array.from(this.distinctKeysIterator("inner", object1, object2, skipShallowCompare)).sort();
    }
    /**
    ...
    static getDifferences(object1, object2, skipShallowCompare = false) {
        return Array.from(this.distinctKeysIterator("full", object1, object2, skipShallowCompare)).sort();
    }
    /**
    ...
    static *distinctKeysIterator(join, object1, object2, skipShallowCompare = false) {
        ...
        if (join === "left") {
            ...
            (0, common_1.assertNever)(join, `Unknown distinctKeysIterator's join param "${join}"`, true);
        }
        for (const key of keys) {
            ...
            if (value1 !== value2) {
                if (!skipShallowCompare &&
                    ...
                    value1 !== null &&
                    value2 !== null &&
                    (0, common_1.isShallowEqual)(value1, value2)) {
                    continue;
                }
                yield key;
            }
...
class AppStateDelta {
    ...
    static calculate(prevAppState, nextAppState) {
        ...
        AppStateDelta.orderAppStateKeys, AppStateDelta.postProcess);
        return new AppStateDelta(delta);
    }
    ...
    squash(delta) {
        ...
        const mergedDeletedSelectedElementIds = Delta.mergeObjects(this.delta.deleted.selectedElementIds ?? {}, delta.delta.deleted.selectedElementIds ?? {});
        const mergedInsertedSelectedElementIds = Delta.mergeObjects(this.delta.inserted.selectedElementIds ?? {}, delta.delta.inserted.selectedElementIds ?? {});
        const mergedDeletedSelectedGroupIds = Delta.mergeObjects(this.delta.deleted.selectedGroupIds ?? {}, delta.delta.deleted.selectedGroupIds ?? {});
        const mergedInsertedSelectedGroupIds = Delta.mergeObjects(this.delta.inserted.selectedGroupIds ?? {}, delta.delta.inserted.selectedGroupIds ?? {});
        const mergedDeletedLockedMultiSelections = Delta.mergeObjects(this.delta.deleted.lockedMultiSelections ?? {}, delta.delta.deleted.lockedMultiSelections ?? {});
        const mergedInsertedLockedMultiSelections = Delta.mergeObjects(this.delta.inserted.lockedMultiSelections ?? {}, delta.delta.inserted.lockedMultiSelections ?? {});
        const mergedInserted = {};
        const mergedDeleted = {};
        if (Object.keys(mergedDeletedSelectedElementIds).length ||
        ...
        if (Object.keys(mergedDeletedSelectedGroupIds).length ||
        ...
        if (Object.keys(mergedDeletedLockedMultiSelections).length ||
    ...
    applyTo(appState, nextElements) {
        try {
            const { selectedElementIds: deletedSelectedElementIds = {}, selectedGroupIds: deletedSelectedGroupIds = {}, lockedMultiSelections: deletedLockedMultiSelections = {}, } = this.delta.deleted;
            const { selectedElementIds: insertedSelectedElementIds = {}, selectedGroupIds: insertedSelectedGroupIds = {}, lockedMultiSelections: insertedLockedMultiSelections = {}, selectedLinearElement: insertedSelectedLinearElement, ...directlyApplicablePartial } = this.delta.inserted;
            const mergedSelectedElementIds = Delta.mergeObjects(appState.selectedElementIds, insertedSelectedElementIds, deletedSelectedElementIds);
            const mergedSelectedGroupIds = Delta.mergeObjects(appState.selectedGroupIds, insertedSelectedGroupIds, deletedSelectedGroupIds);
            const mergedLockedMultiSelections = Delta.mergeObjects(appState.lockedMultiSelections, insertedLockedMultiSelections, deletedLockedMultiSelections);
            const selectedLinearElement = insertedSelectedLinearElement &&
                nextElements.has(insertedSelectedLinearElement.elementId)
                ? new linearElementEditor_1.LinearElementEditor(nextElements.get(insertedSelectedLinearElement.elementId), nextElements, insertedSelectedLinearElement.isEditing)
                : null;
            const nextAppState = {
                ...appState,
                ...
                selectedLinearElement: typeof insertedSelectedLinearElement !== "undefined"
    ...
    filterInvisibleChanges(prevAppState, nextAppState, nextElements) {
        ...
        const prevObservedAppState = (0, store_1.getObservedAppState)(prevAppState);
        const nextObservedAppState = (0, store_1.getObservedAppState)(nextAppState);
        const containsStandaloneDifference = Delta.isRightDifferent(AppStateDelta.stripElementsProps(prevObservedAppState), AppStateDelta.stripElementsProps(nextObservedAppState));
        const containsElementsDifference = Delta.isRightDifferent(AppStateDelta.stripStandaloneProps(prevObservedAppState), AppStateDelta.stripStandaloneProps(nextObservedAppState));
        ...
        if (containsElementsDifference) {
            ...
            const changedElementsProps = Delta.getRightDifferences(AppStateDelta.stripStandaloneProps(prevObservedAppState), AppStateDelta.stripStandaloneProps(nextObservedAppState));
            ...
            if (changedElementsProps.includes("editingGroupId") ||
                ...
                nonDeletedGroupIds = (0, groups_1.getNonDeletedGroupIds)(nextElements);
            }
            // check whether delta properties are related to the existing non-deleted elements
            for (const key of changedElementsProps) {
                switch (key) {
                    case "selectedElementIds":
                        nextAppState[key] = AppStateDelta.filterSelectedElements(nextAppState[key], nextElements, visibleDifferenceFlag);
                        break;
                    case "selectedGroupIds":
                        nextAppState[key] = AppStateDelta.filterSelectedGroups(nextAppState[key], nonDeletedGroupIds, visibleDifferenceFlag);
                        break;
                    ...
                    case "lockedMultiSelections":
                        ...
                        if (!(0, common_1.isShallowEqual)(prevLockedUnits, nextLockedUnits)) {
                            visibleDifferenceFlag.value = true;
                        }
                        break;
                    case "activeLockedId":
                        ...
                        break;
                    default:
                        (0, common_1.assertNever)(key, `Unknown ObservedElementsAppState's key "${key}"`, true);
                }
            }
        }
    ...
    static stripElementsProps(delta) {
        ...
        const { editingGroupId, selectedGroupIds, selectedElementIds, selectedLinearElement, croppingElementId, lockedMultiSelections, activeLockedId, ...standaloneProps } = delta;
    ...
    static postProcess(deleted, inserted) {
        try {
            ...
            Delta.diffObjects(deleted, inserted, "lockedMultiSelections", (prevValue) => (prevValue ?? {}));
        }
        catch (e) {
...
exports.AppStateDelta = AppStateDelta;
/**
...
class ElementsDelta {
    ...
    static satisfiesCommmonInvariants = ({ deleted, inserted, }) => !!(
    ...
    static validate(elementsDelta, type, satifiesSpecialInvariants) {
        for (const [id, delta] of Object.entries(elementsDelta[type])) {
            if (!this.satisfiesCommmonInvariants(delta) ||
    ...
    static calculate(prevElements, nextElements) {
        ...
        for (const prevElement of prevElements.values()) {
            ...
            if (!nextElement) {
                ...
                const inserted = {
                    ...
                    versionNonce: (0, common_1.randomInteger)(),
                };
        ...
        for (const nextElement of nextElements.values()) {
            ...
            if (prevElement.versionNonce !== nextElement.versionNonce) {
                const delta = Delta.calculate(prevElement, nextElement, ElementsDelta.stripIrrelevantProps, ElementsDelta.postProcess);
                if (
        ...
        const applyLatestChangesInternal = (deltas) => {
            ...
            for (const [id, delta] of Object.entries(deltas)) {
                ...
                if (prevElement || nextElement) {
                    latestDelta = Delta.create(delta.deleted, delta.inserted, modifier(prevElement, nextElement), modifierOptions);
                }
                else {
                    latestDelta = delta;
                }
    ...
    applyTo(elements, snapshot = store_2.StoreSnapshot.empty().elements, options) {
        ...
        try {
            const applyDeltas = ElementsDelta.createApplier(elements, nextElements, snapshot, flags, options);
    ...
    squash(delta) {
        ...
        const mergeBoundElements = (prevDelta, nextDelta) => {
            const mergedDeletedBoundElements = Delta.mergeArrays(prevDelta.deleted.boundElements ?? [], nextDelta.deleted.boundElements ?? [], undefined, (x) => x.id) ?? [];
            const mergedInsertedBoundElements = Delta.mergeArrays(prevDelta.inserted.boundElements ?? [], nextDelta.inserted.boundElements ?? [], undefined, (x) => x.id) ?? [];
            if (!mergedDeletedBoundElements.length &&
    ...
    static createGetter = (elements, snapshot, flags) => (id, partial) => {
        ...
        if (!element) {
            ...
            if (element) {
                ...
                element = (0, mutateElement_1.newElementWith)({ id, version: 1 }, {
                    ...partial,
                });
            }
        }
    ...
    static applyDelta(element, delta, flags, options) {
        ...
        return (0, mutateElement_1.newElementWith)(element, directlyApplicablePartial, true);
    }
    /**
    ...
    resolveConflicts(prevElements, nextElements, applyDirection = "forward") {
        ...
        const updater = (element, updates) => {
            ...
            const nextVersion = applyDirection === "forward"
            ...
            if (prevElement === nextElement) {
                ...
                affectedElement = (0, mutateElement_1.newElementWith)(nextElement, {
                    ...elementUpdates,
                    version: nextVersion,
                }, true);
            }
            else {
                affectedElement = (0, mutateElement_1.mutateElement)(nextElement, nextElements, {
                    ...elementUpdates,
                    // don't modify the version further, if it's already different
                    version: prevElement?.version !== nextElement.version
                        ? nextElement.version
                        : nextVersion,
                });
            }
        ...
        for (const [id] of Array.from(Object.entries(this.updated)).filter(([_, delta]) => Object.keys({ ...delta.deleted, ...delta.inserted }).find((prop) => binding_1.bindingProperties.has(prop)))) {
    ...
    static unbindAffected(prevElements, nextElements, id, updater) {
        ...
        binding_1.BoundElement.unbindAffected(nextElements, prevElement(), updater);
        binding_1.BoundElement.unbindAffected(nextElements, nextElement(), updater);
        binding_1.BindableElement.unbindAffected(nextElements, prevElement(), updater);
        binding_1.BindableElement.unbindAffected(nextElements, nextElement(), updater);
    }
    /**
    ...
    static rebindAffected(prevElements, nextElements, id, updater) {
        ...
        binding_1.BoundElement.unbindAffected(nextElements, prevElement(), updater);
        binding_1.BoundElement.rebindAffected(nextElements, nextElement(), updater);
        binding_1.BindableElement.unbindAffected(nextElements, prevElement(), (element, updates) => {
        ...
        binding_1.BindableElement.rebindAffected(nextElements, nextElement(), updater);
    }
    static redrawElements(nextElements, changedElements) {
        try {
            ...
            const tempScene = new Scene_1.Scene(nextElements, { skipValidation: true });
    ...
    static redrawTextBoundingBoxes(scene, changed) {
        ...
        for (const { container, boundText } of boxesToRedraw.values()) {
            ...
            (0, textElement_1.redrawTextBoundingBox)(boundText, container, scene);
        }
    }
    static redrawBoundArrows(scene, changed) {
        for (const element of changed.values()) {
            if (!element.isDeleted && (0, typeChecks_1.isBindableElement)(element)) {
    ...
    static reorderElements(elements, changed, flags) {
        ...
        const ordered = (0, fractionalIndex_1.orderByFractionalIndex)([...unordered]);
        ...
        return (0, common_1.arrayToMap)((0, fractionalIndex_1.syncMovedIndices)(ordered, moved));
    }
    /**
     * It is necessary to post process the partials in case of reference values,
     * for which we need to calculate the real diff between `deleted` and `inserted`.
...
exports.ElementsDelta = ElementsDelta;
