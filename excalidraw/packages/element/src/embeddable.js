...
exports.embeddableURLValidator = exports.maybeParseEmbedSrc = exports.createPlaceholderEmbeddableLabel = exports.getEmbedLink = exports.createSrcDoc = void 0;
...
const RE_YOUTUBE = /^(?:http(?:s)?:\/\/)?(?:www\.)?youtu(?:be\.com|\.be)\/(embed\/|watch\?v=|shorts\/|playlist\?list=|embed\/videoseries\?list=)?([a-zA-Z0-9_-]+)/;
...
const parseYouTubeLikeTimestamp = (url) => {
    let timeParam;
    try {
...
const parseGoogleDriveVideoLink = (url) => {
    try {
        const urlObj = new URL(url.startsWith("http") ? url : `https://${url}`);
        ...
        return {
            fileId,
            resourceKey: resourceKey && /^[a-zA-Z0-9_-]+$/.test(resourceKey)
                ? resourceKey
                : undefined,
...
exports.createSrcDoc = createSrcDoc;
const getEmbedLink = (link) => {
    ...
    if (RE_TWITTER.test(link)) {
        ...
        const safeURL = (0, common_1.escapeDoubleQuotes)(`https://twitter.com/x/status/${postId}`);
        const ret = {
            type: "document",
            srcdoc: (theme) => (0, exports.createSrcDoc)(`<blockquote class="twitter-tweet" data-dnt="true" data-theme="${theme}"><a href="${safeURL}"></a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>`),
    ...
    if (RE_REDDIT.test(link)) {
        ...
        const safeURL = (0, common_1.escapeDoubleQuotes)(`https://reddit.com/r/${page}/comments/${postId}/${title}`);
        const ret = {
            type: "document",
            srcdoc: (theme) => (0, exports.createSrcDoc)(`<blockquote class="reddit-embed-bq" data-embed-theme="${theme}"><a href="${safeURL}"></a><br></blockquote><script async="" src="https://embed.reddit.com/widgets.js" charset="UTF-8"></script>`),
            intrinsicSize: { w: 480, h: 480 },
            sandbox: { allowSameOrigin },
        };
        embeddedLinkCache.set(originalLink, ret);
    ...
    if (RE_GH_GIST.test(link)) {
        ...
        const safeURL = (0, common_1.escapeDoubleQuotes)(`https://gist.github.com/${user}/${gistId}`);
        const ret = {
            type: "document",
            srcdoc: () => (0, exports.createSrcDoc)(`
...
const createPlaceholderEmbeddableLabel = (element) => {
    let text;
    if ((0, typeChecks_1.isIframeElement)(element)) {
        text = "IFrame element";
    }
    else {
        text =
    ...
    const fontFamily = common_1.FONT_FAMILY.Helvetica;
    const fontString = (0, common_1.getFontString)({
        fontSize,
        fontFamily,
    });
    return (0, newElement_1.newTextElement)({
        ...
        fontFamily,
        fontSize,
        text: (0, textWrapping_1.wrapText)(text, fontString, element.width - 20),
        textAlign: "center",
        verticalAlign: common_1.VERTICAL_ALIGN.MIDDLE,
        angle: element.angle ?? 0,
    });
};
exports.createPlaceholderEmbeddableLabel = createPlaceholderEmbeddableLabel;
...
exports.maybeParseEmbedSrc = maybeParseEmbedSrc;
...
exports.embeddableURLValidator = embeddableURLValidator;
