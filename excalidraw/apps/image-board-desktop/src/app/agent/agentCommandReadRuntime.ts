...
export const handleAgentReadCommand = async (
  ...
  switch (request.command) {
    ...
    case "acp.runs": {
      if (!deps.desktopBridge.listAcpAgentRunLogs) {
        throw createAgentCapabilityUnavailableError({
      ...
      return {
        handled: true,
        value: await deps.desktopBridge.listAcpAgentRunLogs({ limit: 50 }),
      };
    }
    case "acp.run": {
      assertAgentProjectPath(request.payload, project.projectPath);
      if (!deps.desktopBridge.readAcpAgentRunLog) {
        throw createAgentCapabilityUnavailableError({
          message: "当前环境不能读取 ACP Agent 任务记录。",
          command: request.command,
      ...
      return {
        handled: true,
        value: await deps.desktopBridge.readAcpAgentRunLog(
          request.payload.taskId,
        ),
      };
    }
    case "acp.threads": {
      if (!deps.desktopBridge.listAcpAgentThreads) {
        throw createAgentCapabilityUnavailableError({
      ...
      return {
        handled: true,
        value: await deps.desktopBridge.listAcpAgentThreads({
    ...
    case "acp.thread": {
      assertAgentProjectPath(request.payload, project.projectPath);
      if (!deps.desktopBridge.readAcpAgentThread) {
        throw createAgentCapabilityUnavailableError({
          message: "当前环境不能读取 ACP Agent 对话记录。",
          command: request.command,
      ...
      return {
        handled: true,
        value: await deps.desktopBridge.readAcpAgentThread(
