...
`image-records.json` 读取后直接通过 TypeScript 类型断言变成 `ImageRecordMap`。Agent 图片入口对 provider 也只是把任意字符串强转成 `ProviderId`。随后生成记录和详情直接调用 `getProviderDefinition(record.provider).label`；目录外 provider 会得到 `undefined`，访问 `.label` 时抛错。由于左右侧栏都位于项目级错误边界内，这类单条记录可能让整个项目视图退回错误页。目录外 provider 对 Codex 来说不一定是坏数据，真正的问题是持久化类型错误地把它收窄成 CoreStudio provider 枚举，展示层又没有安全 fallback。
...
建议：增加一个集中式 `parse/normalizeImageRecordMap`，在项目打开、事务写入和健康检查中复用。可安全兼容的数据做归一化，无法安全推断的数据保留资产并报告健康问题。持久化 provider 应允许外部非空字符串，UI 对目录内 provider 显示友好名称、对目录外 provider 安全显示原值，不能假设它必然属于 CoreStudio 枚举。
...
现有健康检查重点检查生成来源缺失、父链断裂、引用断裂和画板孤儿记录；对于矛盾来源、非法 provider 元数据、key/fileId 不一致、无效时间和尺寸没有对应 issue code。用户在项目还能打开时无法提前发现这些问题，往往要等到 UI 抛错或行为异常。

建议在不删除资产的前提下增加 `invalid-image-record`、`inconsistent-provenance`、`invalid-provider-metadata`、`record-key-mismatch` 等诊断；目录外但格式有效的 provider 不是错误。自动修复只处理可以确定的变换，其余进入人工检查。
