...
- **Canvas Title**（400，24px）：画布利益点标题，尺寸随场景缩放。
- **Canvas Body**（400，18px，1.4 行高）：画布利益点唯一一段事实说明，不再拆出第三层字号。
...
集成中心沿用同一顶栏，但进入独立的 Read mode：桌面端使用宿主/章节索引、正文手册和证据栏三列结构；窄屏按正文优先折叠为单列，并保留可横向浏览的宿主 tabs。页面像一份工作台手册，不使用 SaaS 卡片网格，也不把 WebMCP 能力做成营销徽章。首页只提供轻量 Agent 入口，长教程留在集成中心。
