...
首页把轻量的 CoreStudio 模拟画布组织成一张“工业设计编辑桌”：四张 Dieter Rams 参考素材在左侧组成双排拼贴，右侧按原始 3:2 比例完整展示生成结果，中间以一条 Excalidraw 原生箭头连接。近黑 slogan 与本地项目、模型自由、Agent 写回三个利益点共同构成底部内容带。访客可以选中画布对象、缩放或拖动画布，并触发一次图片生成演示。这些交互只用于展示，不把官网伪装成完整的网页版应用。
