...
- 在“图片资产”面板标题栏点击“网格查看”进入查看模式；网格左上角提供“返回画布”。查看模式以网格浏览画布中的图片，点击打开原图，支持适应窗口、原始尺寸及前后切图；关闭详情返回原滚动位置。浏览不改变原画布布局，也不接受拖拽、批注或编辑命令，外部图片接纳和 Agent 更新继续同步。
- CoreStudio 内使用底部输入框进行单次生成。
- Codex、Cursor、Claude Code、WorkBuddy、千问办公中国版和豆包工作的本地任务负责 Agent 工作流；[接入指南](docs/agent-integration-user-guide.md)说明各宿主浏览器、安装目录和权限差异。
...
项目加载后，把 PNG、JPEG/JFIF、WebP、AVIF、GIF、BMP、ICO 或静态 SVG 放进项目根目录或普通子目录，CoreStudio 会就地登记原图，补齐画布图片和显示缓存。只有根目录 `inbox/` 子树会复制到 `assets/`，并保留 inbox 来源文件。公开整理入口为 `project.json` 的 `layout.order` 与 `layout.elements`，可在外部编辑顺序和坐标；原生场景继续完整保存，应用负责核对与同步。出现冲突时通过“处理文件变化”查看并选择保留版本。原图仍在但两份业务文件都不可用时，重新打开项目可预览并从原图重建。字段、恢复边界与容量证据见[开放项目数据协议](../../../docs/doc/corestudio-open-project-contract.md)。
...
所有图片输入入口共用同一份 [图片格式协议](../../../docs/spec/2026-09-14-corestudio-image-format-protocol.md)。完整扫描规则、异常处理和验收记录见 [外部图片接纳需求](../../../docs/spec/2026-09-05-corestudio-external-image-intake.md)。
