...
| 应用设置 · Codex 集成 | 应用内安装、CLI / Skill / 集成兼容性检测、打开当前项目的轻量引导 | 连接开关、“已连接”状态和任意命令执行入口 |
...
Codex 集成没有启停状态。菜单只负责打开应用设置，欢迎页和画布不再展示重复开关或状态浮窗。Codex 集成页面检测的是本机依赖是否完整，并且只能执行当前签名应用包内的固定零参数安装器，不接受 renderer 提交任意命令、脚本路径或参数。ACP 启停只存在于“实验性功能”。

Codex 集成也不与 CoreStudio 客户端版本一一绑定。客户端版本只记录安装来源；兼容性由独立的 `integrationVersion` 以及 Local Bridge 协议、Skill、CLI 包装器版本共同决定。只有这些集成契约发生变化时才要求更新，普通画板功能、UI 或客户端修复发布不触发重装。旧格式安装记录映射为首个集成版本；只有后续 Skill、CLI 或协议契约确实变化时才提示更新。

安装版 Agent Board 由同一个 Local Bridge 托管静态资源，避免依赖只存在于开发环境的 Vite 地址。匿名状态接口只返回不含项目 token 的 Board 基址；CLI 先用轻量 `read status` 发现会话和项目，再把当前项目 token 补入交给 Codex 打开的 URL。若 Codex 默认网络沙箱阻断 localhost，CLI 会保留“会话已发现”的诊断详情，Skill 据此在沙箱外只重试一次，而不是误报 CoreStudio 未运行。当前 Codex 任务没有实际浏览器控制工具时，Skill 降级为一键链接，不擅自切换其他浏览器。
