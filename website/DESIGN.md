...
- 中央 560 × 420 的 4:3 生成舞台是主视觉，初始、生成中与最终结果只在同一框内切换；左侧参考带放大为 576 × 160，让输入材料先被看清，结果仍以更大的面积承担输出焦点。
...
桌面场景是一张固定的 1680 × 960 编辑平面。560 × 420 的 4:3 生成舞台位于画布中央偏右，并与左侧参考带的垂直中心对齐，是任何状态下都不移动的主对象；四张参考图放大为左侧 576 × 160 的不等宽输入带，并通过一枚 Excalidraw 原生箭头指向舞台。参考带略宽于生成舞台，让输入图片可辨识；结果仍靠更大的纵向面积保持主视觉，但不会吞没整张画布。Slogan 位于左下，以更小的近黑标题场承担次级品牌说明。四格细节带与生成舞台等宽并位于其下方，但首次进入时隐藏。固定于视口的品牌、下载、倍率和 composer 构成真实应用外壳。
