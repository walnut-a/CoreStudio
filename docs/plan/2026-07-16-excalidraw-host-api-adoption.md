...
#### 1. 删除 imperative `scrollToContent` 兼容适配器（已完成）
...
继续保留 imperative 适配器需要长期修改 `App.tsx`、`types.ts` 和兼容测试，但 CoreStudio 已没有实际调用方。2026-07-19 已删除 `ExcalidrawImperativeAPI` 和宿主 API 对该方法的暴露；Excalidraw 内部方法与 `initialData.scrollToContent` 初始化能力保持不变。

#### 2. 补全 Arrange Into Grid 的基线补丁清单（已完成）
...
2026-07-19 已补全清单，并增加基线合同测试，使同步检查和下一次升级评估覆盖完整，不改变现有功能实现。
...
不用于主编辑器工作区。上游 lock 是持续限制平移范围或最低缩放的硬约束，
不符合 CoreStudio 的无限画布语义。

Agent Board 等真正需要固定查看区域的只读场景，将来可以按场景单独采用 lock，不应把它作为全局工作区策略。

2026-07-26 复核确认：CoreStudio 自有的工作区虚线、缩放软停顿和图片落点
`workspaceBounds` 也不再作为主编辑器规则，后续按
`2026-07-26-workspace-fence-removal.md` 删除。图片局部性由批量语义、参考锚点、
participant 视口和最近空位搜索保证，不用另一套软围栏替代上游硬 lock。
...
1. 已完成小型清理改动与基线清单调整。
2. 运行兼容测试、桌面测试、类型检查和构建。
3. 用户完成开发版设置页与核心画布手工回归。
4. 有实际用户功能变更时，再进入版本号和正式发布流程。
