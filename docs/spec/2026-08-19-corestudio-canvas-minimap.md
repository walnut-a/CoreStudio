...
- 在现有 `− / 100% / ＋` 缩放组右侧增加一个与原生 footer 控件同高的 36px 图标按钮，与缩放组保持 8px 间距。
...
- 关闭：普通图标按钮，`aria-pressed="false"`。
- 打开：使用和其他持续开启功能一致的 primary tint，`aria-pressed="true"`。
