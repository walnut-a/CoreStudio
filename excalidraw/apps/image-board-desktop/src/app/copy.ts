...
const zhCnCopy = {
  ...
  applicationSettings: {
    ...
    imageGenerationPage: {
      description: "管理图片生成服务和画布中的生成输入框。",
      composerVisibilityTitle: "画布生成输入框",
      composerVisibilityDescription:
        "控制项目画布底部的图片生成输入框。如果你暂时不需要内置的图片生成功能，可以先关掉；服务、模型和 API Key 会继续保留。",
      composerVisibilityLabel: "显示生成输入框",
      ...
      catalogTitle: "预置模型目录",
      catalogRevision: (revision: number) => `版本 ${revision}`,
      catalogBuiltin: "正在使用应用内置版本",
      catalogRepository: "更新来源",
