...
任务进入完成、失败或取消状态后，输入框里的“日志”入口仍指向同一份保存记录。因为主进程写入 `task.finished` 和刷新 `index.json` 存在一个很短的异步窗口，前端读取保存日志时允许短暂重试；应用设置页如果正在打开，也会在任务结束后自动刷新最近任务列表。
...
- 过程展示采用轻量聊天流，但不做通用聊天产品。用户任务、Agent 回复、工具调用和系统事件各自有清楚角色。
...
当前实现使用 `@assistant-ui/react` 作为成熟聊天组件底座，但只采用它的线程和消息 primitives，不引入默认视觉主题：

- `Conversation`：整个任务过程列表，对应一个 ACP run log。
- `Message`：用户任务、Agent 消息、工具调用、系统状态。
- `Tool / Event`：工具调用、任务状态、错误和协议事件。
- `JSON Inspector`：每条消息下面折叠展示格式化后的原始 payload。

具体接入方式：

- `ExternalThread`：把 CoreStudio 保存的 ACP run log 映射成 assistant-ui 的外部线程消息。
- `ThreadPrimitive.Root / Viewport / Messages`：负责过程流结构、消息上下文和滚动容器。
- `MessagePrimitive.Root`：负责单条消息的基础交互语义，例如稳定的 message id、hover 状态和后续 action bar 扩展点。
- CoreStudio 自己的 CSS：继续控制字号、边框、圆角、颜色和 JSON 折叠样式，确保不偏离 Excalidraw 底座。

默认只展示可读事件；`acp.request`、`acp.response`、`acp.notification` 和 `stderr` 这类协议调试信息默认隐藏。用户点“显示协议 JSON”后，这些事件会进入同一条过程流，而不是另起一套调试表格。这样普通查看和开发排查使用同一个语境。

这套组件应继续使用现有 CSS token、按钮、边框和字号。即使后续继续参考 Vercel AI Elements 或 shadcn Chat Components，也只迁移交互分层和数据组织，不迁移 Tailwind / shadcn 视觉体系。
...
当前实现先在 renderer 侧把已保存的日志条目映射成一层可读聊天流，用来验证信息结构和交互方式；raw ACP traffic 和 stderr 默认隐藏，按需作为协议 JSON 插入同一过程流。后续如果日志格式继续稳定，再把 `displayEvents` 下沉到主进程写入，减少 renderer 对原始日志结构的理解。
...
7. 任务记录详情默认展示中文聊天式过程流，每条记录下可折叠查看格式化 JSON，raw ACP traffic / stderr 默认隐藏。
