  ...
  it("installs and executes the Codex integration from the packaged app", () => {
    ...
    const spawnSync = vi
      ...
      .mockReturnValueOnce({
        status: 0,
        stdout:
          '{"ok":true,"data":{"appVersion":"1.1.26","integrationVersion":"1.6.0","bridgeProtocolVersion":2}}\n',
        stderr: "",
      });
    const rmSync = vi.fn();
    ...
    runCodexIntegrationSmoke({
      ...
      readFileSync: (filePath) =>
        filePath.endsWith("CODEX_INSTALLATION.md")
          ...
          ? '{"installedFromAppVersion":"1.1.26","integrationVersion":"1.6.0","bridgeProtocolVersion":2}'
          : '{"version":"1.1.26"}',
      rmSync,
      spawnSync,
      tmpdir: () => "/tmp",
