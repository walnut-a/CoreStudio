...
2026-09-19 协议补充：项目级事实来源、外部编辑、容错、同步和重建统一见[开放项目数据协议](../doc/corestudio-open-project-contract.md)。本文保留已有接纳行为与历史验收；第 4.3 节的非目标描述本功能原始范围，不限制后续开放整理方向。独立整理文件及其同步尚未实现，不能将新协议视为既有能力。
...
- 支持格式复用 CoreStudio 统一图片输入协议，不在扫描器内维护独立白名单。协议见 [CoreStudio 图片格式协议](./2026-09-14-corestudio-image-format-protocol.md)。
...
不提供专门接纳面板、成功统计、定位新增批次或扫描按钮。首次接纳在已有内容右侧建立起点；后续逐张接续最后一行，达到固定行宽后向下换行，不新建固定分组。后台每次最多处理 8 张只用于资源限额和批次撤销，不决定视觉换行；正常接纳保持静默，不改变视口。

2026-09-19 确认的排布与排序规则：

- 行宽为 6940 个画布单位，可容纳 10 张宽度 640 的横图及其间距。图片沿用最长边最多 640 的等比缩放，横竖间距统一为 60；较窄图片可在同一行放入更多张，下一行从当前行最高图片底部向下留出间距。
- 同一次发现的新增图片按文件创建时间升序排列，相同时间按相对路径稳定排序；创建时间不可用时使用首次发现时间。发现顺序在处理限额截断前保存，重启后继续原队列。后续才发现的图片即使创建时间更早，也追加到末尾，不插入已有图片之间。
- 文件仍在写入时，后面的新图片等待其稳定，避免按解码完成速度插队；失败文件沿用独立退避与修复机制，修复后追加，不重新排列已经成功接纳的图片。
- 当前实现的追加位置以接纳记录中的原始落点延续，不跟随用户后续移动；删除、撤销或移动留下的旧空位不回填，也不复活旧图。避让按当前画布的真实位置和旋转边界进行，只向后寻找空间。后续开放整理按上述协议改为由已接纳的当前整理结果驱动追加，接纳 ledger 不再作为独立排布权威；该调整尚未实现。
- 不随窗口大小、缩放或视口变化重排。旧版项目沿既有接纳区末行继续使用新行宽，保留所有旧元素；全局整理由用户手动进行。


2026-09-19 初版验证（6 张行宽）：32 项接纳与监听定向测试、类型检查及 2418 项桌面测试通过。固定 SOURCE DEV 入口的临时项目实测先接纳 8 张，再加入 4 张，第二批补满第二行；倒序文件名未改变创建时间顺序。回读确认前 8 个元素和视口未变。开发实例、渲染服务和测试进程已收尾；本轮未打包或替换正式版。

2026-09-19 行宽调整：根据用户反馈，将行宽从约 6 张扩大到约 10 张常规横图；32 项定向测试通过，SOURCE DEV 临时项目中 20 张图片按创建时间连续排成两行，每行 10 张，后台批次未造成提前换行。
...
- 打开有效项目，把统一图片输入协议内的图片放入项目根目录或普通子目录，软件就地登记。放入根目录 `inbox/` 子树则保留来源并复制到 `assets/`；不需要预建 inbox。
...
- SVG 只接受可独立解码的静态内容，拒绝脚本、foreignObject、DOCTYPE、外部资源和活动事件；支持内部引用和内嵌的协议内位图。协议外格式不接纳，不影响有效图片。
