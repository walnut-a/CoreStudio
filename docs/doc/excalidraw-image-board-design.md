...
当前 CoreStudio 已把缩略图和预览图作为可重建的派生缓存，`assets/`
中的原图仍是权威数据。Electron `nativeImage` 可直接解码 PNG/JPEG；对画布
能显示但 `nativeImage` 缓冲区无法解码的 WebP/SVG，缓存重建改用系统
缩略图解码作为后备。缓存生成失败不等于原图不可读，健康报告和修复
结果必须分开表达这两种情况。
