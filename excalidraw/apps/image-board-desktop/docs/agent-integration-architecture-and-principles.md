...
| 应用设置 · Codex 集成 | 可复制的自然语言安装请求、CLI / Skill / 集成兼容性检测、打开当前项目的轻量引导 | 连接开关、“已连接”状态和内部脚本参数 |
...
安装版 Agent Board 由同一个 Local Bridge 托管静态资源，避免依赖只存在于开发环境的 Vite 地址。匿名状态接口只返回不含项目 token 的 Board 基址；CLI 在已完成会话发现后，才把当前项目 token 补入交给 Codex 打开的 URL。若 Codex 默认网络沙箱阻断 localhost，CLI 会保留“会话已发现”的诊断详情，Skill 据此在沙箱外重试，而不是误报 CoreStudio 未运行。
