...
describe("getWheelZoomValue", () => {
  ...
  it("allows the host to tune trackpad sensitivity", () => {
    const slowZoom = getWheelZoomValue(1, 10, 0.0035);
    const standardZoom = getWheelZoomValue(1, 10, 0.0065);
    const fastZoom = getWheelZoomValue(1, 10, 0.011);

    expect(slowZoom).toBeGreaterThan(standardZoom);
    expect(standardZoom).toBeGreaterThan(fastZoom);
  });
});
