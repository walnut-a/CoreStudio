...
- 补丁合同已成为强制门禁：当前上游目录的 **53/53 个差异路径全部登记**，未登记路径和未说明的跨组共享会直接失败，失效登记会给出 stale warning。
...
- 补丁路径由实施前 63 个降至 53 个，净减少 10 个，缩减约 **16%**；新增的两个路径用于严格 CSP 下的本地字体策略及其合同测试。
...
仍保留为后续候选、但不阻塞本轮交付的事项：把通用 fallback、搜索禁用语义和剪贴板修复整理为上游候选；评估公开的 Frame 导出 host policy；逐步减少剩余私有 DOM class 接缝。源码开发模式已改为在宿主提供本地资产根时不追加远程字体兜底，消除了 `esm.sh` CSP 警告；构建仍有既有 chunk-size 提示。
