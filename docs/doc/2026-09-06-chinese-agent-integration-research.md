...
> 状态：初次调研已完成；2026-09-07 已实施宿主适配，最新验收见下方实施进度。后续章节保留 09-06 调研时证据。
> 本文中的“官方支持”指 CoreStudio 维护、测试并提供安装与使用入口，不代表腾讯、阿里或字节已认证合作。

## 2026-09-07 实施进度

- 用户已确认三款均登录，并授权三款实机验收；只在 main 工作区修改，未提交、推送或打包。
- 已实现三款宿主 ID、CLI 会话、独立生图权限、托管技能安装和使用说明，复用原有 Local Bridge / Project Room，不修改 Excalidraw 底座。
- WorkBuddy 官方上传探针后实际落盘为 `~/.workbuddy-ai/skills/`，修正此前随包文档中的路径歧义；任务成功加载技能并执行本机命令。
- 千问办公实际加载 `~/.qwenworkcn/skills/` 探针，本机终端通过，暴露 ImageGen；当前任务没有浏览器工具，等待启用官方连接器后复验。
- 豆包工作随包创建指南指定当前环境的 `workspace/.user_skills`，本机默认目录已存在，放入探针后实际任务加载并执行成功；暴露浏览器与图片工具。
- WorkBuddy 与豆包实际浏览器认领、提示词写入、回读已通过，两款独立 session 同时连接同一 Project Room；桌面切换到另一个验收项目后，写入仍留在所认领项目。
- 豆包进一步完成 `read image-paths --visible` 导出 2400×1600 图片并重新导入，回执 `persisted=true`、`roomSequence=persistedSequence=3`，回读确认图片元素 `szn0wna2IXNI4FndZdx62`；其测试浏览器标签已关闭。
- 千问已通过已有的“复制连接指令”流程：独立会话认领并写入文本元素 `2EoFq5EYr5DSX8W4cvILq`，`roomSequence=persistedSequence=4`、`persisted=true`，回读一致。浏览器页由验收方打开并提供真实连接引用，不计为千问自动浏览器验收。实际设置中官方浏览器连接器已安装但关闭，该段仍待启用后验证。
- 定向验证共 147 项通过，类型检查通过。应用保持 1.1.49，Agent integration 为 2.2.0 / Skill 23 / wrapper 2 / Bridge 7；正式安装包尚未构建，未调用原生或 CoreStudio 付费生图。
- 界面证据：开发版六宿主设置页与豆包实际内置浏览器画布均已截图展示。WorkBuddy 浏览器截图也已检查，其画布文本绘制于 canvas，不能用 DOM 文本匹配作为可见性断言。
...
| 项目 | WorkBuddy 5.5.2 | 千问办公 1.0.1 | 豆包工作 2.25.18 |
| --- | --- | --- | --- |
| 实际技能加载、本机终端 | 通过 | 通过 | 通过 |
| 宿主独立 session、认领与提示词持久化 | 通过 | 通过，使用真实复制连接引用 | 通过 |
| 实际浏览器读取 nonce | 通过，agent-browser 自行打开 | 通过，官方 Chrome 连接器读取已连接的测试标签页；自行新开标签未单独验收 | 通过，内置浏览器自行打开 |
| 可见参考图导出并作为图片写回 | 共享 CLI/权限回归覆盖，本宿主未单独跑图片往返 | 共享 CLI/权限回归覆盖，本宿主未单独跑图片往返 | 实机通过，2400×1600 JPEG |
| 原生生图 | 当前任务未提供工具 | 暴露工具，未调用 | 暴露工具，未调用 |
| 正式包一键安装 | 隔离 HOME 安装器测试通过；正式包未构建 | 同左 | 同左；未初始化目录会明确停止 |

所有真实写入均位于临时验收项目 `/private/tmp/corestudio-intake-20260905/接纳乙`，没有写入正式用户项目。验收时桌面另开 `/private/tmp/CoreStudio-Agent-Validation-20260907`，证明写回目标不随人的标签焦点变化。不得把上述结果写成三款全部自动浏览器、生图或正式包验收通过。

收尾：三款临时探针均已移除；测试 CLI、连接引用文件与临时导出图片已清理。WorkBuddy 自动化浏览器、豆包测试标签和验收方浏览器标签均已关闭；固定 SOURCE DEV 进程组 95175 已停止，5174 / 9331 / 60910 均无监听。保留原有三款宿主 App 与正式版，不清理用户登录状态。临时 QA 项目保留供复验。

### 千问浏览器启用状态复查（2026-09-07 01:16）

用户表示已启用后，重新进入千问客户端并刷新连接器列表：浏览器已安装，但开关仍显示 off（截图也为灰色）。另新建任务 `Check browser connector tools`，实际查询 MCP 工具注册表返回 0 个工具，未提供浏览器控制函数；HTTP 请求 CoreStudio 测试页面正常，不能据此算浏览器渲染通过。随后用户授权直接开启客户端浏览器开关，点击后进入“等待扩展连接”。Chrome 当前配置中搜索“千问”和“QwenWork”均无结果；客户端提供的官方扩展目录为 `~/Library/Application Support/QwenWorkCN/chrome-extension`，版本 1.5.8。该次检查停在扩展权限确认；随后用户明确授权安装并说明已经安装，继续按当前配置核验，结果如下。

### 千问官方浏览器连接复验通过（2026-09-07）

用户授权后，在 Chrome 当前 Walnut 配置中从客户端提供的官方目录加载 QwenWork 1.5.8，扩展列表显示启用。不能据此推断其他浏览器配置从未安装。扩展在 `chrome://extensions` 页面显示“受限页面”；切换到本次专用 CoreStudio 测试标签页并点击“连接”后显示“已连接”。千问客户端连接器开关随后开启，新任务成功加载实际浏览器工具。

任务 `Execute bounded integration test` 使用 `mcp__builtin_browser__tabs_context` 定位本次已连接标签页，使用 `mcp__builtin_browser__javascript_tool` 读取真实 DOM nonce，再执行 qwenwork session 连接、Board 认领、项目路径校验及一次提示词写入。写入元素 `l1xuiO9AZJeDcxcfSXHz4`，operationId `ef45a919-10e3-4646-a564-cd346fcc0d2d`，persistedSequence 1，`persisted: true`。验收方通过相同绑定的 CLI 独立重读 scene，确认内容 `qwenwork browser validation` 存在且未删除；真实截图确认页面已经进入画布。

本轮补齐“宿主实际浏览器读取页面 → 认领 → CLI 持久化”链路，没有进行收费生图，没有把复用已连接标签页算成自行新开标签验收。官方扩展与客户端开关按用户要求保留启用。

复验收尾：本次 Chrome 测试标签页及扩展管理标签页已关闭；固定 SOURCE DEV 进程组 59158 已停止且 remaining=0，临时 CLI 与测试指令文件已移除。保留原有宿主应用、Chrome 和正式 CoreStudio，测试数据仍保留在临时 QA 项目。

### 2026-09-07 接入说明全面同步

已同步应用中英文设置说明与各宿主首次使用指令、官网中英文六宿主指南和 WebMCP 枚举、共用 Skill 与三款宿主附录、CLI 合同、用户指南、架构边界和文档索引。Skill 修订升至 24，集成 2.2.0，Bridge 与 CLI wrapper 不变。确认当前分发源是 CoreStudio 随包资源；项目链接和当前 GitHub 账号没有发现独立 CoreStudio 技能仓库，因此未臆造外部仓库或改动个人已安装技能。

验证：32 项定向测试、TypeScript 类型检查、Skill 格式校验通过；中英文官网合同验证通过；SOURCE DEV 与官网真实页面截图覆盖新增宿主及移动端。未提交、推送、部署或打包，线上站点和用户已安装应用仍需后续交付更新。

本轮收尾：固定 SOURCE DEV 进程组 4899 已由标准入口停止，remaining=0；本次网站预览服务 PID 3907 已停止，预览标签已关闭，视口尺寸已还原。官网合同测试 12 项通过。
