...
首页把轻量的 CoreStudio 模拟画布组织成一张“工业设计编辑桌”：近黑标题场和大幅生成结果构成封面，Dieter Rams 参考素材压成单行素材带；同一结果的细节裁切、模型与 Agent 标注在用户完成生成后再出现。访客可以选中画布对象、缩放或拖动画布，并触发一次图片生成演示。这些交互只用于展示，不把官网伪装成完整的网页版应用。
...
`dieter-rams-*.webp` 是四张独立的画布参考图片，`corestudio-canvas-result-rams-v2.webp` 是与这些参考在材质、配色上对应的演示生成结果。参考图保持为无圆角的独立图片对象，不预合成为网页卡片；结果细节带只复用这张结果图的不同裁切，不伪造多轮生成。旧版产品截图仍保留在 `assets/` 中，但当前首页不再加载它们。
