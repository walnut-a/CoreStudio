...
node bin/corestudio.cjs scene add-image <path> --task-id <id> --write-token <token> --json
node bin/corestudio.cjs scene add-prompt --text "..." --task-id <id> --write-token <token> --json
node bin/corestudio.cjs generate --prompt "..." --use-selection --task-id <id> --write-token <token> --json
node bin/corestudio.cjs task complete --task-id <id> --write-token <token> --json
```
