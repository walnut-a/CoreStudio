...
镜头以当前视口的安全区计算构图倍率。90% 是常规桌面镜头的上限参考，不再作为所有桌面窗口的固定下限；普通桌面与 471–820px 紧凑窗口优先完整展示标题、参考带和主结果，超宽或高分辨率屏幕再提高倍率并向上收紧空白。470px 以下保留可读的 40% 聚焦镜头，让标题完整出现并露出结果关系，不把整张 1680 × 960 场景压缩到无法阅读。画布继续支持单指平移、双指以手势中心缩放和点击选中。最小倍率以完整场景适配视口为准，并受生产级缩放下限约束。
